# Warehouse Order Batching Productively with Python 📦

In a **Distribution Center (DC)**, walking between locations during the picking route can consume 60–70% of an operator’s working time. Minimizing this walking time is one of the most effective ways to boost overall DC productivity.
...
This project presents a model designed to simulate the effects of various picking processes and routing strategies, aiming to determine the optimal order-picking approach. It leverages the **Single Picker Routing Problem (SPRP)** within a two-dimensional warehouse layout (x-axis, y-axis).

The SPRP is a specialized form of the **Traveling Salesman Problem (TSP)**, which asks:
>  “Given a set of storage locations and the distances between them, what is the shortest route that visits each location and returns to the depot?”

This repository includes a ready-to-use **Streamlit application** for **Logistics Engineers** to evaluate different strategies simply by uploading their own dataset of order line records.
